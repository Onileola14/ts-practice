...
let scoresByStudent: { [studentName: string]: number } = {
  Ada: 90,
  Chidi: 85,
};
scoresByStudent["Onileola"] = 95; // ✅ new dynamic key, still type-checked

// 6. Reusing a shape with a type alias (cleaner than repeating inline objects)
type Product = {
  id: string;
  price: number;
  inStock: boolean;
...
let laptop: Product = { id: "p-01", price: 899, inStock: true };
printProduct(laptop);
